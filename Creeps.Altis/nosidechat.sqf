			...
			if (ctrlText ((findDisplay 63) displayCtrl 101) == "Side Channel") then 
			{
				...
				if (disconnect_me >= 1) then {
					if (isNil "warn_last") then {
						...
						[] spawn DS_slap_them;
						//["beat04"] spawn DS_really_loud_sounds;
						["Du wurdest gewarnt. Denk 20 Sekunden darüber nach, ob es das wert war."] spawn DS_double_cut;
						1 fademusic 10;
						1 fadesound 10;
						disableUserInput true;
						systemChat ("Du wurdest gewarnt. Denk 20 Sekunden darüber nach, ob es das wert war.");
						sleep 20;
