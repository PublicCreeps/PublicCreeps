...
_veh = nearestObjects[(getPos _station),["Car","Air"],15];

if((count _veh) < 1) exitWith { hint format["Kein Fahrzeug in der naehe."]; life_action_inUse = false; };
...
// Price per Litre
// _indFuel = ([sell_array,"oilp"] call BIS_fnc_findNestedElement) select 0;
// life_fuelstation_price = ([(((sell_array select _indFuel) select 1) / 200) * _pricemod, 2] call BIS_fnc_cutDecimals) max 3;
life_fuelstation_price = ([((3500) / 200) * _pricemod, 2] call BIS_fnc_cutDecimals) max 3;

_txtVeh ctrlShow false;
_txtFit ctrlSetText format["Zapfsäule $%1 Liter",life_fuelstation_price];
...

{
	...
	//diag_log format["%1 %2 %3 %4 %5 %6",_x,_type,_info,_owner,_kap,_diff];
	/*
