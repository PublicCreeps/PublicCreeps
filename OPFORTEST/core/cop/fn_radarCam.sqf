...
if (_speed > _limit) then {
	if (side _driver == civilian) then {
		...
		if(license_civ_driver OR license_civ_truck ) then {
			...
			if(life_speedTicket <= 3) then {systemChat format["Dies ist der %1. Strafzettel für dich, pass auf oder du musst nächstes mal mehr bezahlen.",life_speedTicket];};
			if(life_speedTicket == 4) then {systemChat format["Dies ist der %1. Strafzettel für dich, beim nächsten mal verlierst du einen Führerschein!",life_speedTicket];};
			if(life_speedTicket > 4 && life_speedTicket < 8) exitWith {
					
					systemChat format["Dies ist der %1. Strafzettel für dich.",life_speedTicket];
			...
			if(life_speedTicket > 8) exitWith {
					...
					systemChat format["Das war dein %1th Strafzettel",life_speedTicket];
			...
			if(life_cash <= _ticket) then
			{
				...
				{
					hint parseText format ["<t color='#ffffff'><t size='2'><t align='center'>Speed Radar<br/><t color='#ff0000'><t align='center'><t size='1.5'>Speed: %1 km/h<br/><t color='#ffffff'><t align='center'><t size='1'>Speed Limit: %2 km/h<br/><t color='#ffffff'><t align='center'><t size='1'>Fahrer: %3<br/><t color='#ffffff'><t align='center'><t size='1'>Strafe: $%4<br/>Since you are broke and cannot pay, you now have a warrant out for you instead",round _speed,_limit,name _driver,[_ticket] call life_fnc_numberText];
					[[getPlayerUID _driver,name _driver,"120S"],"life_fnc_wantedAdd",false,false] spawn life_fnc_MP;
				};
				life_atmcash = life_atmcash - _ticket;
				hint parseText format ["<t color='#ffffff'><t size='2'><t align='center'>Speed Radar<br/><t color='#ff0000'><t align='center'><t size='1.5'>Speed: %1 km/h<br/><t color='#ffffff'><t align='center'><t size='1'>Speed Limit: %2 km/h<br/><t color='#ffffff'><t align='center'><t size='1'>Fahrer: %3<br/><t color='#ffffff'><t align='center'><t size='1'>Strafe: $%4",round _speed,_limit,name _driver,[_ticket] call life_fnc_numberText];
				...
				[0] call SOCK_fnc_updatePartial;
					...
					if(life_speedCaught > 6) exitWith {
							...
							hint parseText format ["<t color='#ffffff'><t size='2'><t align='center'>Speed Radar<br/><t color='#ff0000'><t align='center'><t size='1.5'>Speed: %1 km/h<br/><t color='#ffffff'><t align='center'><t size='1'>Speed Limit: %2 km/h<br/><t color='#ffffff'><t align='center'><t size='1'>Fahrer: %3<br/>Du wirst nun wegen mehrmaligen fahren mit überhöter Geschwindigkeit gesucht.",round _speed,_limit,name _driver];
							[[getPlayerUID _driver,name _driver,"120FSWL"],"life_fnc_wantedAdd",false,false] spawn life_fnc_MP;
							
					};
					hint parseText format ["<t color='#ffffff'><t size='2'><t align='center'>Speed Radar<br/><t color='#ff0000'><t align='center'><t size='1.5'>Speed: %1 km/h<br/><t color='#ffffff'><t align='center'><t size='1'>Speed Limit: %2 km/h<br/><t color='#ffffff'><t align='center'><t size='1'>Fahrer: %3<br/><t color='#ffffff'><t align='center'><t size='1'>Du wirst jetzt wegen fahren ohne Führerschein gesucht",round _speed,_limit,name _driver];
