...
[] spawn
{
	while {true} do
	{
		...
		while{(life_drug > 0)} do {
			...
			if(life_drug > 0.10) then {

			sleep 240;
			life_drug = life_drug - 0.02;
			} else {
...
[] spawn {
	...
	while{true}do{
		...
		_tanke = nearestObjects [player, ["Land_fs_feed_F"], 500];

		if(count _tanke != 0)then{

			{ 
				_x setFuelCargo 0;

				if(!(_x in _tankstellen))then{
					_tankstellen set [count _tankstellen,_x];
